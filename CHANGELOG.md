...
- **Feature**: Web analytics visitors can be tied to a contact. `NotifuseAnalytics.identify(email, hmac)` attaches a verified contact address to a session, and visitors arriving from a tracked email link are identified automatically with no code. The signature is checked against the workspace secret and the address must already belong to a contact, so the public tracking endpoint can never create contacts or store a stranger's email. Enable "Record web goals on the contact timeline" in Web Analytics settings to have identified goals appear on the contact's timeline, where they can trigger automations and change segment membership.
