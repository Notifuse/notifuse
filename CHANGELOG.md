...
- **Change**: The anonymous telemetry payload gains one boolean, `web_analytics`, reporting whether a workspace recorded a web analytics session in the last 30 days. It measures actual use rather than the settings toggle, since the feature can be switched on and never wired up. The date it derives from never leaves your installation, and `TELEMETRY=false` still sends nothing at all.
