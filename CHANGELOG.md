...
- **Fix**: Deleting a workspace no longer strips the application database user's privileges on the *system* database. `REVOKE ... ON ALL TABLES/ALL SEQUENCES IN SCHEMA public` applies to the database the connection is attached to, and these statements were issued over the system-database connection — so deleting a workspace emptied the ACLs of the system database's own tables, after which authentication and workspace lookups failed with `permission denied for table users` until an operator re-granted by hand. Deployments running as a superuser (the `postgres` default) were unaffected in practice, since superusers bypass privilege checks. The revoke block is gone entirely: the workspace database is now removed with a single `DROP DATABASE ... WITH (FORCE)`, which also terminates leftover backends atomically instead of terminating them, sleeping 100 ms and hoping no connection is re-established before the drop. On PostgreSQL 12 and older, which cannot parse `WITH (FORCE)`, the previous terminate-then-drop sequence is used as a fallback (with the database name bound as a parameter rather than interpolated). The database name is now also quoted rather than interpolated raw, and lower-cased before quoting to match the name PostgreSQL actually stored when the unquoted `CREATE DATABASE` folded it — without that, an uppercase `DB_PREFIX` or workspace ID would make the quoted drop match nothing and silently orphan the database. Finally, deleting a workspace is no longer bound to the caller's request context: an aborted `DROP DATABASE` can leave the database marked invalid — unconnectable, not recreated automatically, and recoverable only by a manual drop — and a client disconnecting midway could previously drop the database while leaving the workspace rows behind, so that the next request recreated the workspace empty. Note that this prevents further damage but does not repair an installation already affected; to restore a system database whose privileges were stripped, connect to it and run `GRANT ALL ON ALL TABLES IN SCHEMA public TO <db_user>; GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO <db_user>;` (the table owner can do this without superuser) (#396)
