...
- **Feature**: Web navigation on the contact timeline. Each identified visitor's sessions and pageviews appear in their timeline — path, engaged time, scroll depth, entry and exit pages, with the visit summarised in a single entry that fills in as they browse. Navigation entries never trigger automations, and they are usable in segment conditions: the Activity condition offers both, and filters on the event itself — page path, time on page, scroll depth, entry and exit pages, UTM, device, country. There is no setting to switch on and none to forget: identify() requires your workspace secret, so the decision is made in your own code. Obtaining consent to store browsing history against a contact remains yours to do.
