...
- **Feature**: Audit logs. Logs → Audit logs shows who did what in a workspace — a member invited, a permission set changed, an API key created, an integration edited, a template saved, a broadcast scheduled, a contact deleted — with the address it came from, the request it belonged to, and whether it succeeded, failed or was refused. The security-sensitive entries carry a before/after of what changed, with every password, secret and key replaced by `[redacted]`; the fact that they changed is kept, the values are not. Failures are recorded too: a sign-in for an unknown address, a wrong or expired code, a request refused by a permission check or by the licence, each with the reason the response deliberately does not give away. What is never recorded is the data plane — contact upserts, sends, tracking, custom events — which belongs in Logs, not in a trail.
  ...
  Entries live in the system database, keyed by workspace, so deleting a workspace deletes nothing from the log: the record of who deleted it, and of what they did before, survives. The table is append-only — a database trigger refuses every UPDATE and TRUNCATE, and every DELETE except the retention purge — so no code path in Notifuse, and no stray statement through its database role, rewrites history. It does not stop the owner of the database, who can disable the trigger, and the docs say so rather than pretending otherwise. A root user can switch the tab to the whole deployment: every workspace plus the deployment-level entries — sign-ins, the licence, the system settings.

  Reading the log is an opt-in grant. Owners always see it; a member or an API key sees it only when given the new Audit logs read permission, which is deliberately not part of "full access" — full access should not imply reading every colleague's email address, IP address and refused actions, and adding it to full access would have turned every existing full-access member into a restricted one on upgrade. The tab filters by date, action, outcome, actor, target and IP, pages by cursor, and exports the current filter as CSV or NDJSON (up to 100 000 rows; the export is itself recorded). Retention defaults to 365 days; an owner sets the workspace's own in Settings → Audit logs (0 keeps forever, otherwise 30 to 3650 days) and a root user sets the deployment default in the system settings. Each purge that removed something is recorded. Also available as `auditLogs.list`, `auditLogs.get`, `auditLogs.actions`, `auditLogs.export` and `workspaces.setAuditLogSettings`.

- **Change**: Audit logs are the sixth Licensed Feature, in the Enterprise tier. The gate is on **recording** and on nothing else: without a key covering `audit_logs` nothing is written, and nothing is refused — the page, the export and the retention setting stay available and show whatever was recorded while a key covered it. A key that lapses past its 30-day grace stops recording and writes one marker entry, `licence.recordingStopped`, so the gap is never silent; installing a key writes `licence.recordingResumed`. The startup log says `audit_logs_recording=false` when this applies. Enterprise keys issued since v40 already carry `audit_logs`, so nothing has to be re-issued. The LICENSE file of v41 and later names the sixth capability; v40 and earlier are unchanged, as the licence promises. The retention purge never consults the licence: deleting data must not depend on it.

- **Improvement**: Every API response now carries an `X-Request-ID` header, generated when the client sent none and echoed when it did. It is stored on every audit entry, so an entry can be tied back to the application log line that served it.
...
- **Fix**: The healthchecks no longer hardcode values the configuration can change. `pg_isready -U postgres` named the user literally, so an install with a custom `DB_USER` probed as a role that does not exist — `pg_isready` reports the server up either way, because it never authenticates, but PostgreSQL logs a rejected connection for every probe, one every five seconds. It follows `POSTGRES_USER` and `POSTGRES_DB` now. The API healthcheck probed a literal `8080` while `SERVER_PORT` was configurable, so a custom port left the container permanently unhealthy; the healthcheck and the published container port both follow `SERVER_PORT` now.
