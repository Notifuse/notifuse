...
- **Feature**: Web Analytics — the Staminads feature set, rebuilt on PostgreSQL. A ~21 KB gzipped cookieless browser SDK (`/na.js`) streams sessions, pageviews and goals into monthly-partitioned tables per workspace, with engaged-time metrics (TimeScore, bounce rate), editable channel-attribution rules (40 defaults, with historical backfill), goals with a declared type and value, 10 custom dimensions, geo resolution from a bundled MaxMind GeoLite2 City database, and a console section with Dashboard, Explore, Goals, Filters and Settings tabs plus a live view. The v38 migration adds the tables to existing workspaces and grants the new web analytics permission to existing members and pending invitations; the feature is off until enabled in a workspace's Web Analytics settings. For high-traffic installs, `compose.alloydb.yaml` runs the same schema on AlloyDB Omni's columnar engine.

- **Feature**: Web analytics visitors can be tied to a contact. `NotifuseAnalytics.identify(email, hmac)` attaches a verified contact address to a session, and visitors arriving from a tracked email link are identified automatically with no code. The signature is checked against the workspace secret — only your own server can mint one — and an address it vouches for becomes a contact if it is not one already, carrying the country, language and timezone the visit reported. An existing contact is never modified by a visit. Calling identify() is itself the opt-in: an identified visitor's goals appear on the contact's timeline, where they can trigger automations and change segment membership. `trackGoal` requires the goal's type — purchase, subscription, lead, signup, booking, trial or other — because only your site knows which it is, and an untyped conversion cannot be used by the goal-based segment conditions. A workspace that wants anonymous reporting simply never calls it.

- **Feature**: Web navigation on the contact timeline. Each identified visitor's sessions and pageviews appear in their timeline — path, engaged time, scroll depth, entry and exit pages, with the visit summarised in a single entry that fills in as they browse, and any page openable in a side panel. Navigation entries never trigger automations, and they are usable in segment conditions: the Activity condition offers both, and filters on the event itself — page path and domain, time on page, scroll depth, entry and exit pages, UTM, device, country. There is no setting to switch on and none to forget: identify() requires your workspace secret, so the decision is made in your own code. Obtaining consent to store browsing history against a contact remains yours to do.

- **Feature**: An AI assistant on the Web Analytics section. Ask it questions about your own traffic and it queries the same reports the dashboard draws, so its answers and the charts beside them can never disagree; ask it to summarise a period and it pulls the headline metrics, the trend and the leading breakdowns in one go and tells you what actually moved. It can also drive the screen — change the period or comparison, apply filters, open an Explore report — and it will say when the data does not explain a change rather than inventing a reason. It needs an LLM integration configured under Settings → Integrations, and it never groups or filters by visitor email address.

- **Feature**: Annotations on the Web Analytics charts. Mark the moments that explain a curve — a launch, a price change, an outage — with a title, a colour and an optional note, and they appear as labelled verticals on the dashboard and goal charts, each pinned to the timezone it was entered in so "9am in Tokyo" reads back as 9am. Broadcasts annotate themselves: one is added the moment a send starts. The new Annotations tab lists them all, and lets you add, edit and delete your own.
...
- **Fix**: Saving blog settings no longer erases the SEO canonical URL. The console never loaded the stored value into the form, so every save sent it back empty and the blog's `<link rel="canonical">` tag disappeared. The field is now populated from the saved settings.

- **Fix**: Tracked email links no longer lose query parameters. Appending UTM parameters rewrote the entire query string, which silently dropped pairs Go's URL parser rejects (`?sid=1;2`, `?discount=50%off`), reordered the rest and re-escaped literal characters — so a recipient could land on a URL missing the parameter the link depended on. Links are now left byte-for-byte as authored apart from the parameters Notifuse itself appends. Per-link click reporting groups by the rewritten URL, so a link whose bytes change starts a new row from this release while its existing history stays under the old one.
- **Fix**: Links written as `<a HREF="…">` are tracked again. The rewriter matched the attribute case-sensitively, so uppercase markup — routine in content pasted from Word or Outlook — got no click tracking, no UTM parameters and no identity token, while lowercase links in the same email were rewritten normally.
...
- **Fix**: Single sign-on reports a conflicting identity link correctly on a PostgreSQL server running a non-English locale. Duplicate rows were recognised by matching the driver's English error text, which PostgreSQL translates according to `lc_messages` — so on a French or Japanese server an account already linked to another SSO identity was reported as a generic "sign-in failed, please try again", and re-running the setup wizard failed instead of accepting the root user it had already created. Detection now keys off the SQLSTATE, which no locale changes.
...
- **Fix**: Listing, editing and deleting transactional notifications now require the Transactional permission. The three endpoints checked workspace membership only, so a member whose transactional access had been revoked could still read the notifications, repoint one at a different template and delete it — the console offered the permission and the API ignored it.
