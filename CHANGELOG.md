...
- **Security**: Cleared the eleven open Dependabot alerts. `google.golang.org/grpc` moves to 1.83.2 in both Go modules, closing a crash in gRPC's xDS server on a request with no `:authority` or `Host` header; Notifuse runs no xDS server, but the module version is what a scanner matches on. The three front-ends move to `vitest` 4.1.11, because the arbitrary-file-read fix in `@vitest/mocker` has no 3.x release, and to `js-yaml` 4.3.2; both are build-time only, and `npm audit` is clean in all three.
